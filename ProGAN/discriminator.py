...
def minibatch(input:Tensor) -> Tensor:
    constant = torch.std(input, dim=0).mean().repeat(input.shape[0], 1, input.shape[2], input.shape[3])
...
class Discriminator(nn.Module):
    def __init__(self) -> None:
        ...
        for i in range(3):
            self.layerlist.append(DoubleConv(2**(i+6), 2**(i+7), pixnorm=False, inv=True))
        for i in range(3):
            self.layerlist.append(DoubleConv(512, 512, pixnorm=False, inv=True))
